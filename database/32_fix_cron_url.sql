-- ============================================================================
-- 32_fix_cron_url.sql — SUPERSEDED BY 33_set_cron_secrets.sql. DO NOT RUN.
--
-- Kept only so the numbered sequence has no hole and the reasoning below stays
-- on the record.
--
-- 33 redefines `call_cron_endpoint` with everything this file added — the same
-- rtrim/ltrim normalisation and the same timeout — PLUS a check that the URL
-- actually has a scheme, which this version lacked. Running 32 after 33 would
-- therefore be a downgrade: it would silently remove that guard and put back
-- the "Bad scheme" failure mode from three frames inside pg_net.
--
-- If you are replaying migrations from scratch, skip straight from 31 to 33.
--
-- ---------------------------------------------------------------------------
-- THE BUG (still accurate, and 33 carries the fix)
--
...
-- The former contents of this file, commented out so replaying the folder in
-- order cannot re-apply the weaker function. See 33_set_cron_secrets.sql.
--
